...
- 通过 WebSocket 实时查看设备状态和下卡日志，断线后自动重连。
...
首次刷入公共固件后，连接 `Air780E-xxxxxx` 热点（无需密码），打开 `http://192.168.4.1/`，填写 2.4 GHz Wi-Fi 和面板访问密钥。保存后设备重启，从路由器查看设备地址，在浏览器打开 `http://设备地址/`。

开机 30 秒未连上 Wi-Fi 会自动进入配网；设置页也可点击“进入配网”。重新配网需填写原访问密钥，其他设置保留。
